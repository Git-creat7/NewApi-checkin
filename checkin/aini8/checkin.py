...
SESSION = os.getenv("AINI8_SESSION", "").strip()
...
def make_session(include_api_user: bool = True) -> requests.Session:
    if not SESSION:
        raise ApiError("AINI8_SESSION is required.")

    session = requests.Session(impersonate="chrome124", timeout=TIMEOUT)
    session.cookies.set("session", SESSION, domain="api.aini8.com")
...
def fetch_self(session: requests.Session) -> dict:
    ...
    if API_USER and actual_id and API_USER != actual_id:
        raise ApiError(
            f"AINI8_API_USER={API_USER} 与当前登录账号 id={actual_id} 不一致，请改成 {actual_id}。"
        )
    return user
...
def fetch_checkin_status(session: requests.Session) -> dict:
    ...
    return ensure_json_response(response, "AINI8 签到状态响应")
...
def post_checkin(session: requests.Session) -> dict:
    ...
    return ensure_json_response(response, "AINI8 签到动作响应")
